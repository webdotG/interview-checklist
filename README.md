...
5. Если НЕ получил ответ - НЕ ставишь галочку
6. НО ЕСЛИ ХОЧЕШЬ НАПИСАТЬ СВОЙ КОММЕНТАРИЙ НАПРИМЕР НА РЕАКЦИЮ ЧЕЛОВЕКА ТО ЭТО МОЖНО СДЕЛАТЬ  
   (ответ засчитан не будет а комментарий будет)
7. После всех вопросов жмешь "Отправить ответы"
8. Результаты сохраняются к тебе на компьютер

## Структура ответа для наглядности

````json
# Структура данных интервью

## Формат JSON, сохраняемый в Firebase Firestore:
...
- **answers** - вложенная структура ответов:
  - **Категория** (например, "Технические вопросы")
    - **Подкатегория** (например, "О проекте и архитектуре")
      - **Вопрос** (текст вопроса)
        - **checked** - отмечен ли вопрос (boolean)
        - **note** - заметка к вопросу (string или null)

## Особенности:

- Сохраняются только **отмеченные** вопросы (`checked: true`)
- Заметки могут быть пустыми (`null`)
- Структура позволяет легко группировать вопросы по категориям
- Timestamp автоматически генерируется Firebase (`serverTimestamp()`)
